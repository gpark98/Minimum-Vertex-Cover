...
def write_files(graph_name, k, mvc, total_time):
    ...
    # Write solution file
    f = open(graph_name + "_Approx.sol","w+")
    f.write(str(k) + "\n")
    for i in range(k):
    ...
    # Write trace file
    f = open(graph_name + "_Approx.trace","w+")
    f.write(str(total_time) + ", " + str(k))
    f.close()


# Running through each graph
...
# for i in graph_list:
#     graph_name = i
#     graph_file = "Desktop/DATA/" + graph_name + ".graph" # update your file path here
#     G = read_graph(graph_file)
#     max_degree = G.degree
#     k, mvc, total_time = MDG(G, 1000) # Used 1000 seconds
#     write_files(graph_name, k, mvc, total_time)
