...
Sasha M. Bakker, Sebastian Gutierrez, Gyujin Park and Nathan B. Williams
...
def vert_min_degree(V,G,N= 1):
    ...
    for x in V:
        ...
        try:
            list_degrees.append(G.degree[x])

        except: 

            continue
    sort = np.argsort(np.array(list_degrees))
...
def genetic_algorithm_min_vertex_opt(n_bits: int , n_iter: int, n_pop: int, r_cross: float,r_mut: float ,num_edges_G: int, G, random_seed,max_time):
    '''
    Input:
    ...
    # Variables for storing time of solution improvement and size of new vertex cover
    ...
    best_eval = cost_cover(num_edges_G,n_bits,current_best,G)
    ...
    while(best_eval < 1 and sum(time_res)<max_time and k >= sum(best)):

        # print('as')

        # print(cost_cover(num_edges_G,n_bits,current_best,G),k)
    ...
    print(new_vertex_cov)

    # if len(time_res)> 1:

    #     return k,V,time_res[:-1],new_vertex_cov[:-1]

    # else:


    return k,V,time_res,new_vertex_cov